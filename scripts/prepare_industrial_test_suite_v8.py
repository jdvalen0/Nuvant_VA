import cv2
import os
import numpy as np
import shutil
import random
from pathlib import Path

# --- CONFIGURACIÓN V8 (ZERO-ASSUME) ---
SOURCE_DIR = Path("/home/juan-david-valencia/Escritorio/Nuvant_VA/images (2)")
OUTPUT_DIR = Path("/home/juan-david-valencia/Escritorio/Nuvant_VA/INDUSTRIAL_TEST_SUITE_V8")

# LISTAS BLANCAS INMUTABLES (Hardcoded & Verified)
SOURCES = {
    # 1. ANILLOS: 2385-2394 (10 archivos). 
    # ERROR PREVIO: Se incluía 2395 que era cuero. CORREGIDO.
    "REF_01_Anillos": {
        "type": "img_list",
        "files": [f"IMG_{i}.JPEG" for i in range(2385, 2395)]
    },
    
    # 2. TRAMA FINA: 2402-2415 (Excluyendo 2410).
    # ERROR PREVIO: Se incluía Lisa (2420) que es cuero. CORREGIDO.
    # ERROR PREVIO: Se incluía 2410 que tiene puntos blancos. CORREGIDO.
    "REF_02_Trama": {
        "type": "img_list",
        "files": [f"IMG_{i}.JPEG" for i in [
            2402, 2403, 2404, 2405, 2406, 2407, 2408, 2409, 
            # SKIP 2410
            2411, 2412, 2413, 2414, 2415
        ]]
    },

    # 3. PIEL A (RUGOSA): 05:01 - 05:50
    # CONFIRMADO VISUALMENTE.
    "REF_03_Piel_Rugosa": {
        "type": "frame_range_explicit",
        "start": "frame_20260115-050100",
        "end": "frame_20260115-055000"
    },

    # 4. PIEL B (SUAVE): 06:20 - 06:40
    # CONFIRMADO VISUALMENTE.
    "REF_04_Piel_Suave": {
        "type": "frame_range_explicit",
        "start": "frame_20260115-062000",
        "end": "frame_20260115-064000"
    },

    # 5. PIEL C (INTERMEDIA): 06:50 - 07:00
    # AJUSTE V8: Movidor atrás desde 07:11 que era muy oscuro.
    "REF_05_Piel_Intermedia": {
        "type": "frame_range_explicit",
        "start": "frame_20260115-065000",
        "end": "frame_20260115-070000"
    },

    # 6. GEOMETRICA: 12:50 - 14:50
    # AJUSTE V8: Inicio movido de 12:30 a 12:50 para evitar desenfoque.
    "REF_06_Geometrica": {
        "type": "frame_range_explicit",
        "start": "frame_20260115-125000",
        "end": "frame_20260115-145000"
    },

    # 7. RADIAL: 17:35 - 23:59
    # CONFIRMADO VISUALMENTE.
    "REF_07_Radial": {
        "type": "frame_range_explicit",
        "start": "frame_20260115-173500",
        "end": "frame_20260115-235959"
    }
}

def apply_augmentation(img, forceful=False):
    augs = [img.copy()]
    # Rotaciones seguras (textura uniforme)
    augs.append(cv2.rotate(img, cv2.ROTATE_90_CLOCKWISE))
    augs.append(cv2.rotate(img, cv2.ROTATE_180))
    augs.append(cv2.flip(img, 1)) # Horizontal flip
    
    # Cambios de brillo LEVES para robustez, pero sin cambiar el color base
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    v = hsv[:,:,2].astype(np.int16)
    
    shifts = [10, -10] if not forceful else [20, -20, 30] 
    for s in shifts:
        v_mod = np.clip(v + s, 0, 255).astype(np.uint8)
        new_hsv = hsv.copy()
        new_hsv[:,:,2] = v_mod
        augs.append(cv2.cvtColor(new_hsv, cv2.COLOR_HSV2BGR))
        
    return augs

def apply_industrial_defect(image, defect_type='stain'):
    img = image.copy()
    h, w = img.shape[:2]
    
    # Defectos sintéticos claros para validar la detección
    if defect_type == 'hole':
        center = (random.randint(50,w-50), random.randint(50,h-50))
        cv2.circle(img, center, random.randint(15,35), (10,10,10), -1)
        
    elif defect_type == 'stain_dark':
        mask = np.zeros((h,w), dtype=np.uint8)
        center = (random.randint(50,w-50), random.randint(50,h-50))
        cv2.circle(mask, center, random.randint(30,70), 255, -1)
        mask = cv2.GaussianBlur(mask, (31,31), 0)
        overlay = img.copy()
        overlay = (overlay * 0.5).astype(np.uint8) # Darker stain
        alpha = (mask / 255.0)[:,:,np.newaxis]
        img = (img * (1 - alpha) + overlay * alpha).astype(np.uint8)
        
    return img

def main():
    print(f"--- 🔒 Generando V8 ZERO-ASSUME ---")
    if OUTPUT_DIR.exists(): shutil.rmtree(OUTPUT_DIR)
    
    all_frames = sorted([f for f in os.listdir(SOURCE_DIR) if f.startswith("frame_")])

    for ref_name, cfg in SOURCES.items():
        print(f"📦 Procesando {ref_name}...")
        ref_path = OUTPUT_DIR / ref_name
        for sub in ["01_Entrenamiento_NORMAL", "02_Validacion_BUENAS", "03_Validacion_ADULTERADAS"]:
            (ref_path / sub).mkdir(parents=True, exist_ok=True)
            
        raw_imgs = []
        
        # COLECCIÓN DE FUENTES
        if cfg["type"] == "img_list":
            for fname in cfg["files"]:
                p = SOURCE_DIR / fname
                if p.exists():
                    i = cv2.imread(str(p))
                    if i is not None: raw_imgs.append(i)
                else:
                    print(f"   ⚠️ Warning: Falta {fname}")
                    
        elif cfg["type"] == "frame_range_explicit":
            candidates = [f for f in all_frames if f >= cfg["start"] and f <= cfg["end"]]
            print(f"   - Frames en rango: {len(candidates)}")
            # Sampleo inteligente: Tomar cuadros distribuidos uniformemente
            # Max 50 muestras para no sobrecargar
            if len(candidates) > 0:
                step = max(1, len(candidates) // 50)
                selection = candidates[::step]
                for f in selection:
                    i = cv2.imread(str(SOURCE_DIR / f))
                    if i is not None: raw_imgs.append(i)

        if not raw_imgs:
            print(f"❌ ERROR CRÍTICO: No hay imágenes para {ref_name}. Revisar Config.")
            continue

        print(f"   - Fuente Limpia Verificada: {len(raw_imgs)} imágenes.")

        # GENERACIÓN DE DATASETS
        
        # 1. Entrenamiento (30) - Augmentación Fuerte
        count = 0
        attempts = 0
        while count < 30 and attempts < 200:
            attempts += 1
            base = random.choice(raw_imgs)
            # Solo rotar, no queremos cambiar el patrón base
            aug = random.choice(apply_augmentation(base, forceful=True))
            cv2.imwrite(str(ref_path / "01_Entrenamiento_NORMAL" / f"train_{count:02d}.png"), aug)
            count += 1
        
        # 2. Validación Buenas (15) - Augmentación Mínima (Casi idéntico a producción)
        count = 0
        attempts = 0
        while count < 15 and attempts < 100:
            attempts += 1
            base = random.choice(raw_imgs)
            # Espejo o nada. Queremos ver si el modelo reconoce la tela tal cual.
            aug = random.choice([base, cv2.flip(base, 1)]) 
            cv2.imwrite(str(ref_path / "02_Validacion_BUENAS" / f"val_ok_{count:02d}.png"), aug)
            count += 1
            
        # 3. Validación Adulterada (15) - Defectos Sintéticos sobre Fuente Limpia
        count = 0
        attempts = 0
        while count < 15 and attempts < 100:
            attempts += 1
            base = random.choice(raw_imgs)
            aug = base.copy()
            bad = apply_industrial_defect(aug, random.choice(['stain_dark', 'hole']))
            cv2.imwrite(str(ref_path / "03_Validacion_ADULTERADAS" / f"val_fail_{count:02d}.png"), bad)
            count += 1
            
    print("\n✅ V8 COMPLETADO. Listo para inspección visual humana.")

if __name__ == "__main__":
    main()
